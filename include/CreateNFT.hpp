...
class NftBox : public QObject {
  ...
  Q_PROPERTY(
      QString issuer READ issuerBech32 WRITE setIssuer NOTIFY issuerChanged)
  Q_PROPERTY(QString address READ addressBech32 NOTIFY addressChanged)
  Q_PROPERTY(
      QString metdata READ metdata WRITE setMetdata NOTIFY metdataChanged)
...
public:
  NftBox(QObject *parent = nullptr)
      : QObject(parent), m_selected(false), m_state(Stte::Ready),
        m_outId(QString::number(index++)) {}
  NftBox(std::shared_ptr<const Output> out, QObject *parent = nullptr,
         c_array outId = c_array());

  enum Stte { Minting, Sending, Burning, Ready };
  Q_ENUM(Stte)
  ...
  QString metdata() const {
    return (m_data.isEmpty()) ? QString() : QString(m_data);
  }
  ...
  void setIssuer(QString addr) {
    if (NftBox::set_addr(addr, m_issuer))
      emit issuerChanged();
  }
