...
NftBox::NftBox(std::shared_ptr<const Output> out, QObject *parent,
               c_array outId)
    : NftBox(parent) {
  m_outId = outId;
  ...
  connect(info, &Node_info::finished, this, [=]() {
    const auto issuefea =
        out->get_immutable_feature_(qblocks::Feature::Issuer_typ);
    if (issuefea) {
      m_issuer = std::static_pointer_cast<const Issuer_Feature>(issuefea)
                     ->issuer()
                     ->addr();
      m_issuerBech32 =
          qencoding::qbech32::Iota::encode(info->bech32Hrp, m_issuer);
      emit issuerChanged();
    }
    const auto immetfea =
        out->get_immutable_feature_(qblocks::Feature::Metadata_typ);
    if (immetfea) {
      m_data =
          std::static_pointer_cast<const Metadata_Feature>(immetfea)->data();
    ...
    m_addressBech32 =
        qencoding::qbech32::Iota::encode(info->bech32Hrp, m_address);
    emit addressChanged();
    info->deleteLater();
  });
}
...
void NftBox::mint() {
  ...
  connect(info, &Node_info::finished, this, [=]() {
    if (Wallet::instance()->addresses().size()) {
      ...
      const auto unlock = Unlock_Condition::Address(
      ...
      const auto deposit =
          NodeConnection::instance()->rest()->get_deposit(NFTout, info);
      NFTout->amount_ = deposit;
      pvector<const Output> theOutputs{NFTout};

      if (m_issuer.isNull() || Wallet::instance()->addresses().find(m_issuer) !=
                                   Wallet::instance()->addresses().cend()) {
        InputSet inputSet;
        ...
        if (!m_issuer.isNull()) {
          ...
          if (issuerAddr->type() == Address::NFT_typ ||
              issuerAddr->type() == Address::Alias_typ) {
            consumedAmount +=
                Wallet::instance()->consume(inputSet, stateOutputs1, deposit,
                                            {Output::All_typ}, {addB->outId()});
          }
        }
        ...
        for (const auto &v : std::as_const(stateOutputs1)) {
          ...
          const auto cdep =
              NodeConnection::instance()->rest()->get_deposit(out, info);
          out->amount_ = cdep;
          stateAmount += cdep;
        ...
        if (consumedAmount < requiredAmount) {
          consumedAmount += Wallet::instance()->consume(
              inputSet, stateOutputs2, 0, {Output::Basic_typ},
              {}); // Fix this set the amount need it
        }
        ...
        if (consumedAmount >= requiredAmount) {
          ...
          auto payloadusedids =
              Wallet::instance()->createTransaction(inputSet, info, theOutputs);
          auto block = Block(payloadusedids.first);

          const auto transactionid =
              payloadusedids.first->get_id().toHexString();
          auto res = NodeConnection::instance()->mqtt()->get_subscription(
...
void NftBox::burn() {
  ...
  auto info = NodeConnection::instance()->rest()->get_api_core_v2_info();
  connect(info, &Node_info::finished, this, [=]() {
    if (Wallet::instance()->addresses().find(m_address) !=
        Wallet::instance()->addresses().cend()) {
      const auto unlock = Unlock_Condition::Address(
          Wallet::instance()->addresses().begin()->second->getAddress());

      const auto maininputs =
          Wallet::instance()->addresses().at(m_address)->inputs();
      std::set<c_array> outids;
      for (auto i = maininputs.cbegin(), end = maininputs.cend(); i != end;
           ++i) {
        outids.insert(i.key());
      }
      ...
      quint64 consumedAmount = 0;
      quint64 stateAmount = 0;
      consumedAmount += Wallet::instance()->consume(inputSet, stateOutputs1, 0,
                                                    {Output::NFT_typ}, outids);

      for (auto i = stateOutputs1.cbegin(), end = stateOutputs1.cend();
           i != end; ++i) {
        ...
        if (i.key() != m_outId) {
          ...
          out->unlock_conditions_ = {
              unlock}; // Fix: Add state transition is if alias
          const auto cdep =
              NodeConnection::instance()->rest()->get_deposit(out, info);
          out->amount_ = cdep;
          stateAmount += cdep;
      ...
      for (const auto &v : std::as_const(stateOutputs2)) {
        ...
        out->consume();
        out->unlock_conditions_ =
            prevUnlocks; // Fix: Add state transition is if alias
        const auto cdep =
            NodeConnection::instance()->rest()->get_deposit(out, info);
        out->amount_ = cdep;
        stateAmount += cdep;
...
void BoxModel::sendAll(QString recAddr) {
  ...
  connect(info, &Node_info::finished, this, [=]() {
    ...
    if (NftBox::set_addr(recAddr, recArray)) {
      const auto addrUnlock =
          Unlock_Condition::Address(Address::from_array(recArray));
      pvector<const Output> theOutputs;
      const auto maininputs =
          Wallet::instance()->addresses().begin()->second->inputs();
      std::set<c_array> outids;
      for (auto i = maininputs.cbegin(), end = maininputs.cend(); i != end;
           ++i) {
        outids.insert(i.key());
      ...
      for (auto i = stateOutputs1.cbegin(), end = stateOutputs1.cend();
        ...
        out->consume();
        out->unlock_conditions_ = {
            addrUnlock}; // Fix: Add state transition is if alias
      ...
      if (consumedAmount >= stateAmount + minDeposit) {
        ...
        auto res = NodeConnection::instance()->mqtt()->get_subscription(
            "transactions/" + transactionid + "/included-block");
        QObject::connect(res, &ResponseMqtt::returned, this,
                         [=](auto var) { res->deleteLater(); });
...
void BoxModel::sendSelecteds(QString recAddr, QDateTime unixTime) {
  ...
  connect(info, &Node_info::finished, this, [=]() {
    ...
    if (NftBox::set_addr(recAddr, recArray)) {
      ...
      if (unixTime.isValid()) {
        const auto expirUnlock = Unlock_Condition::Expiration(
            unixTime.toSecsSinceEpoch(),
            Wallet::instance()->addresses().begin()->second->getAddress());
      ...
      for (auto i = 0; i < boxes.size(); i++) {
        if (boxes[i]->selected()) {
          ...
          box->setState(NftBox::Stte::Sending);
          const auto maininputs =
              Wallet::instance()->addresses().at(box->addrArray())->inputs();
          for (auto i = maininputs.cbegin(), end = maininputs.cend(); i != end;
               ++i) {
            outids.insert(i.key());
            routids.insert(i.key());
          }
          QTimer::singleShot(20000, box,
                             [=]() { box->setState(NftBox::Ready); });
        }
      }
      ...
      for (auto i = stateOutputs1.cbegin(), end = stateOutputs1.cend();
        ...
        if (soutids.find(i.key()) != soutids.cend()) {
          ...
          if (routids.find(i.key()) != routids.cend()) {
            out->unlock_conditions_ = {
                unlock}; // Add state transition is if alias
          } else {
            out->unlock_conditions_ =
                prevUnlock; // Add state transition is if alias
          }
        }
        const auto cdep =
            NodeConnection::instance()->rest()->get_deposit(out, info);
  ...
  connect(info, &Node_info::finished, this, [=]() {
    ...
    if (NftBox::set_addr(recAddr, recArray)) {
      ...
      const auto unlock = Unlock_Condition::Address(
          Wallet::instance()->addresses().begin()->second->getAddress());
      pset<const Unlock_Condition> theUnlocks{addrUnlock};
      auto expirUnlock = Unlock_Condition::Expiration(
          unixTime.toSecsSinceEpoch(),
          Wallet::instance()->addresses().begin()->second->getAddress());
      if (unixTime.isValid()) {
        // Add storage unlock to recieve the extra from adding the expiration?
      ...
      quint64 neededAmount =
          (amount >= minDeposit) ? amount : (minDeposit + amount);

      if (amount < minDeposit) {
        const auto varstoraUnlock = Unlock_Condition::Storage_Deposit_Return(
            Wallet::instance()->addresses().begin()->second->getAddress(),
            minDeposit);

        pset<const Unlock_Condition> varUnlocks{addrUnlock, varstoraUnlock};
        if (!unixTime.isValid()) {
          expirUnlock = Unlock_Condition::Expiration(
        ...
        const auto storaUnlock = Unlock_Condition::Storage_Deposit_Return(
            Wallet::instance()->addresses().begin()->second->getAddress(),
            minDeposit - amount);
      ...
      if (neededAmount < sendamount)
        neededAmount = sendamount;
      ...
      for (auto i = stateOutputs1.cbegin(), end = stateOutputs1.cend();
        ...
        auto prevUnlock = out->unlock_conditions_;
        out->consume();
        out->unlock_conditions_ = prevUnlock; // Add state transition is if
                                              // alias
        const auto cdep =
            NodeConnection::instance()->rest()->get_deposit(out, info);
        out->amount_ = cdep;
...
int BoxModel::count() const { return boxes.size(); }
...
bool BoxModel::setData(const QModelIndex &index, const QVariant &value,
                       int role) {

  if (index.row() < boxes.size()) {
    const auto re =
        boxes[index.row()]->setProperty(roleNames().value(role), value);
...
QModelIndex BoxModel::index(int row, int column,
...
void BoxModel::addBox(NftBox *o) {
  ...
  connect(o, &NftBox::issuerChanged, this, [=] {
    ...
    if (ind > -1)
      emit dataChanged(index(ind), index(ind),
                       QList<int>{ModelRoles::issuerRole});
  });
  connect(o, &NftBox::metdataChanged, this, [=] {
    const auto ind = idToIndex(o->outId());
    if (ind > -1)
      emit dataChanged(index(ind), index(ind),
                       QList<int>{ModelRoles::metdataRole});
  });
  connect(o, &NftBox::addressChanged, this, [=] {
    ...
    if (ind > -1)
      emit dataChanged(index(ind), index(ind),
                       QList<int>{ModelRoles::addressRole});
  });
